...
class Order extends Component {
  ...
  componentDidMount() {
    this.getJumlahPesanan(
      moment()
        .add(this.state.tanggalDitambahkan, 'days')
        .format('YYYY-MM-DD')
        .toString()
    );
  }
  ...
  cekHari = () => {
    if (
      moment().add(this.state.tanggalDitambahkan, 'days').format('dddd') ===
      'Sunday'
    ) {
            ...
            {this.state.detailShow === true ? (
              ...
              <td>
                <input
                  ...
                  className="btn btn-info"
                  value="Detail"
                  onClick={() => this.getDetailPesanan(val)}
                />
              </td>
            )}
          </tr>
      ...
      <div>
        <div className="card m-5">
          <div
            ...
            style={{ backgroundColor: '#E32E89' }}
          >
            <h3>ORDER LIST</h3>
          </div>
          <div className="card-body">
            <div className="row">
              <div className="col-5">{this.cekHari()}</div>
              <div className="col-7 text-right">
                <div className="row">
                  <div className="col-6">
                    {this.state.tanggalDitambahkan === 0 ? (
                      <button
                        className="btn btn-dark btn-block p-1 m-0"
                        disabled
                      >
                        Back
                      </button>
                      ...
                      <>
                        {moment()
                          .add(this.state.tanggalDitambahkan - 1, 'days')
                          .format('dddd') === 'Sunday' ? (
                          <button
                            className="btn btn-success btn-block p-1 m-0"
                            onClick={() =>
                              this.setState({
                                tanggalDitambahkan:
                                  this.state.tanggalDitambahkan - 3,
                                getData: false,
                              })
                            }
                          >
                          ...
                          <button
                            ...
                            onClick={() =>
                              this.setState({
                                tanggalDitambahkan:
                                  this.state.tanggalDitambahkan - 1,
                                getData: false,
                              })
                            }
                          >
            ...
            {this.state.seluruhPesananHariIni.length > 0 ? (
              <Table striped bordered hover className="mt-4">
                ...
                <tbody>{this.renderListPesanan()}</tbody>
              </Table>
            ) : (
